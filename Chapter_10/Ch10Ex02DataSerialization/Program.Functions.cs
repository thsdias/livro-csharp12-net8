...
{
    static void GenerateXmlFile(IQueryable<Category> categories, bool useAttributes = true)
    {
    ...
    static void GenerateJsonFile(IQueryable<Category> categories)
    {
        ...
        {
            using (Utf8JsonWriter json = new(jsonStream, new JsonWriterOptions { Indented = true }))
            {
