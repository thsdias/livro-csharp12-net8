...
using Microsoft.EntityFrameworkCore.Metadata.Internal; // To use XElement, XAttribute.
...
partial class Program
{
    private static void FilterAndSort()
    {
        ...
        PrintSQLQuery(projectedProducts);
    ...
    private static void AggregateProducts()
    {
        ...
        if (db.Products.TryGetNonEnumeratedCount(out int countDbSet))
            WriteLine($"{"\nProduct count from DbSet:",-25} {countDbSet, 10}");
        else
            WriteLine("\nProdcuts DbSet does not have a Count property.");

        // Try to get an efficient count from a List<T>.
        List<Product> products = db.Products.ToList();

        if (products.TryGetNonEnumeratedCount(out int countList))
            WriteLine($"{"\nProduct count from list:",-25} {countList, 10}");
        else 
    ...
    private static void FilterAndSortExtension()
    {
        SectionTitle("Filter and sort");
    ...
    private static void PrintSQLQuery(IQueryable<object> obj)
    {
        WriteLine();
        WriteLine(obj.ToQueryString()); // SQL Query.
        WriteLine();
    }
}