...
drives.AddColumn($"[{color}]NAME[/]");
drives.AddColumn($"[{color}]TYPE[/]");
drives.AddColumn($"[{color}]FORMAT[/]");
drives.AddColumn(new TableColumn($"[{color}]SIZE (BYTES)[/]").RightAligned());
drives.AddColumn(new TableColumn($"[{color}]FREE SPACE[/]").RightAligned());
...
WriteLine($"Temporary File Name: {GetTempFileName()}"); // creates a zero-byte file and returns its name, ready for you to use.
WriteLine();
#endregion
